...
export default function HomeClient({ tools }: HomeClientProps) {
  const [filterDimensions, setFilterDimensions] = useState<Record<string, any>>({});
  const [typeDimension, setTypeDimension] = useState<string[]>([]);
      ...
      return Object.keys(activeFilters).every((filter) => {
        ...
        if (typeof filterValue === "object" && filterValue) {
          return filterValue.every((value: string) => {
            if (!tool[filter as keyof Tool]) {
              return false;
            }
            return (tool[filter as keyof Tool] as string).split(", ").includes(value);
          });
        } else {
          return filterValue === tool[filter as keyof Tool];
        }
      });
    });
